...
int ProjectileMonsterDamage(Missile &missile)
{
	...
	return monster.mMinDamage + GenerateRnd(monster.mMaxDamage - monster.mMinDamage + 1);
}
...
bool GuardianTryFireAt(Missile &missile, Point target)
{
	...
	AddMissile(position, target, dir, MIS_FIREBOLT, TARGET_MONSTERS, missile._misource, dmg, missile.sourcePlayer()->GetSpellLevel(SPL_GUARDIAN), &missile);
